...
void CStage5::Update()
{
	...
	if (m_goal.GetGoal() != true)
	{
		...
		{
			ExecuteChangeCamera(ARRAYSIZE(collision5InfoTable2D), ARRAYSIZE(collision5InfoTable3D));
		}
		else
		{
			if (!(GAMEFLG->Getflg()))
...
void CStage5::Draw()
{
	...
	m_Ground5.Draw(m_camera.GetViewMatrix(), m_camera.GetProjectionMatrix());//ステージ１を描画
...
void CStage5::CreateCollision3D()
{
	int arraySize = ARRAYSIZE(collision5InfoTable3D);
	if (arraySize >= MAX_COLLISION)
	{
		std::abort();
	}
	for (int i = 0; i < arraySize; i++) {
		SCollisionInfo& collision = collision5InfoTable3D[i];
		//ここで剛体とかを登録する。
		//剛体を初期化。
		{
...
void CStage5::CreateCollision2D()
{
	int arraySize = ARRAYSIZE(collision5InfoTable2D);
	if (arraySize >= MAX_COLLISION)
	{
		std::abort();
	}
	for (int i = 0; i < arraySize; i++) {
		SCollisionInfo& collision = collision5InfoTable2D[i];
		//ここで剛体とかを登録する。
		//剛体を初期化。
		{
			//この引数に渡すのはボックスのhalfsizeなので、0.5倍する。
