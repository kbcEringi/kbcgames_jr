...
void CStage9::Initialize()
{
	...
	m_goal.Initialize(D3DXVECTOR3(232.0f, 10.0f, 0.0f));
...
void CStage9::Update()
{
	...
	if (m_goal.GetGoal() != true)
	{
		...
		{
			ExecuteChangeCamera(ARRAYSIZE(collision9InfoTable2D), ARRAYSIZE(collision9InfoTable3D));
		}
		else
		{
			if (!(GAMEFLG->Getflg()))
...
void CStage9::Draw()
{
	...
	m_Ground9.Draw(m_camera.GetViewMatrix(), m_camera.GetProjectionMatrix());//ステージ１を描画
...
void CStage9::CreateCollision3D()
{
	int arraySize = ARRAYSIZE(collision9InfoTable3D);
	if (arraySize >= MAX_COLLISION)
	{
		std::abort();
	}
	for (int i = 0; i < arraySize; i++) {
		SCollisionInfo& collision = collision9InfoTable3D[i];
		//ここで剛体とかを登録する。
		//剛体を初期化。
		{
...
void CStage9::CreateCollision2D()
{
	int arraySize = ARRAYSIZE(collision9InfoTable2D);
	if (arraySize >= MAX_COLLISION)
	{
		std::abort();
	}
	for (int i = 0; i < arraySize; i++) {
		SCollisionInfo& collision = collision9InfoTable2D[i];
		//ここで剛体とかを登録する。
		//剛体を初期化。
		{
			//この引数に渡すのはボックスのhalfsizeなので、0.5倍する。
