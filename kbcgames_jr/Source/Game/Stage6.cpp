...
SCollisionInfo collision6InfoTable3D[] = {
#include "Collision3D_stage06.h"
};

SCollisionInfo collision6InfoTable2D[] = {
...
D3DXVECTOR3 playerpos_stage6 = {
#include"Player_stage6.h"
};
...
void CStage6::Initialize()
{
	...
	m_Player.SetPosition(playerpos_stage6);
	m_Ground6.Initialize();
	...
	m_goal.Initialize(D3DXVECTOR3(216.0f, 29.0f, 0.0f));
...
void CStage6::Update()
{
	m_Player.Died(playerpos_stage6);

	if (m_goal.GetGoal() != true)
	{
		if (GAMEPAD(CGamepad)->GetConnected())
		{
			ExecuteChangeCamera(ARRAYSIZE(collision6InfoTable2D), ARRAYSIZE(collision6InfoTable3D));
		}
		else
		{
			if (!(GAMEFLG->Getflg()))
...
void CStage6::Draw()
{
	...
	m_Ground6.Draw(m_camera.GetViewMatrix(), m_camera.GetProjectionMatrix());//ステージ１を描画
...
void CStage6::CreateCollision3D()
{
	int arraySize = ARRAYSIZE(collision6InfoTable3D);
	if (arraySize >= MAX_COLLISION)
	{
		std::abort();
	}
	for (int i = 0; i < arraySize; i++) {
		SCollisionInfo& collision = collision6InfoTable3D[i];
		//ここで剛体とかを登録する。
		//剛体を初期化。
		{
...
void CStage6::CreateCollision2D()
{
	int arraySize = ARRAYSIZE(collision6InfoTable2D);
	if (arraySize >= MAX_COLLISION)
	{
		std::abort();
	}
	for (int i = 0; i < arraySize; i++) {
		SCollisionInfo& collision = collision6InfoTable2D[i];
		//ここで剛体とかを登録する。
		//剛体を初期化。
		{
			//この引数に渡すのはボックスのhalfsizeなので、0.5倍する。
